#ifndef OMP_MINIMAX_CONTENTION_SOLVER_H
#define OMP_MINIMAX_CONTENTION_SOLVER_H
...
// Directly copied from SequentialMinimaxSolver, including the global
// best_score, best_move which leads to lots of cache contention, resulting in
// slower performance than the sequential.
template <class Game, int depth>
class OmpMinimaxContentionSolver : public GameSolver<Game> {
...
template <class Game, int depth>
void OmpMinimaxContentionSolver<Game, depth>::playBestMove() {
    switch (game_.status()) {
...
template <class Game, int depth>
int OmpMinimaxContentionSolver<Game, depth>::playBestMoveForGame(
...
#endif  /* OMP_MINIMAX_CONTENTION_SOLVER_H */
